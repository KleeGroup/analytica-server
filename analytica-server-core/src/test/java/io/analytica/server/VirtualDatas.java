...
public class VirtualDatas {
	...
	public void loadVisitors(final Date startDate, final double visitorByHour) throws InterruptedException {
		...
		for (int j = 4; j >= 0; j--) {
			...
			for (int h = 7; h < 19; h++) {
				...
				final double coef = 0.25 + 0.25 * Math.sin((h - 7 + 4.5) * Math.PI / 3); //varie de 0 é 0.5
				//if (h == 10 || h == 15) {
				//	nbVisit = Math.round(NB_VISIT_DAILY_MAX * 0.6d);
				//} else {
				//	nbVisit = Math.round(NB_VISIT_DAILY_MAX * 0.2d);
				nbVisit = random(Math.round(NB_VISIT_DAILY_MAX * 0.3d), coef * 2); // de 30% é 60% en fonction de l'heure
				//}
	...
	private static long nextGaussian(final double moyenne, final double maxValue) {
		Assertion.checkArgument(moyenne >= 1, "La moyenne doit étre supérieure ou égale é 1");
		Assertion.checkArgument(maxValue > moyenne, "La valeur max doit étre supérieure é la moyenne");
