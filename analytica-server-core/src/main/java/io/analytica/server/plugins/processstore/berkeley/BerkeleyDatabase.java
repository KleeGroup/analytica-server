...
final class BerkeleyDatabase {
		...
		try {
			...
			try {
				...
				if (lastKey.getSize() == 0) { //si lastKey est null, on veut le premier élément
					...
					//Si on a dejé un lastKey, on repositionne le cursor dessus, puis on fait next()
	...
	void open(final boolean readOnly) {
		try {
			...
			//On limite l'utilisation du cache é 20% de la mémoire globale.
			myEnvConfig.setCachePercent(20);
			//CHECKME On limite l'utilisation du cache é 200Mo
