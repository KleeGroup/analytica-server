...
public class InfluxDBProcessAggregatorPlugin implements ProcessAggregatorPlugin {
	...
	@Override
	public void push(final Identified<AProcess> process) throws ProcessAggregatorException {
	...
	public String getLastInsertedProcess(final ProcessAggregatorQuery aggregatorQuery) {
		try {
			final InfluxDBSimpleDataQuery dataQuery = new InfluxDBSimpleDataQuery(aggregatorQuery);
			final Connector connector = getConnector(aggregatorQuery.getAggregatorDataFilter().getApplicationName());
			connector.flush();
			final List<ProcessAggregatorDto> results = connector.execute(dataQuery);
			final boolean databaseIsEmpty = results.isEmpty();
			if (databaseIsEmpty) {
				return null;
			}
	...
	public List<ProcessAggregatorDto> findAllLocations(final ProcessAggregatorQuery aggregatorQuery) throws ProcessAggregatorException {
		final InfluxDBTagQuery tagQuery = new InfluxDBTagQuery(aggregatorQuery, InfluxDBQuery.TAG_LOCATION, aggregatorQuery.getAggregatorDataSelector());
	...
	public List<ProcessAggregatorDto> findAllTypes(final ProcessAggregatorQuery aggregatorQuery) throws ProcessAggregatorException {
		final InfluxDBMeasurementsQuery measurementsQuery = new InfluxDBMeasurementsQuery(aggregatorQuery.getAggregatorDataSelector());
	...
	public List<ProcessAggregatorDto> findAllCategories(final ProcessAggregatorQuery aggregatorQuery)
		...
		final InfluxDBTagQuery tagQuery = new InfluxDBTagQuery(aggregatorQuery, InfluxDBQuery.TAG_CATEGORY, aggregatorQuery.getAggregatorDataSelector());
		return getConnector(aggregatorQuery.getAggregatorDataFilter().getApplicationName()).execute(tagQuery);
	}
	...
	public List<ProcessAggregatorDto> getTimeLine(final ProcessAggregatorQuery aggregatorQuery) throws ProcessAggregatorException {
		final InfluxDBDataQuery dataQuery = InfluxDBQueryFactory.getDataQuery(aggregatorQuery);
	...
	public static InfluxDB getDB(final String httpAddresse, final String port, final String username, final String password) throws ProcessAggregatorException {
		...
		if (ping(httpAddresse)) {
			final InfluxDB influxDB = InfluxDBFactory.connect(httpCompleteAdresse, username, password);
	...
	private Connector getConnector(final String applicationName) throws ProcessAggregatorException {
