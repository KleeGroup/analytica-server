 ...
 */
public final class RestQueryNetApiPlugin implements QueryNetApiPlugin, Activeable {
	private static final Logger LOG = Logger.getLogger(RestQueryNetApiPlugin.class);
	 ...
	 */
	@Inject
	public RestQueryNetApiPlugin(@Named("httpPort") final int httpPort) {
	...
	private final HttpServer startServer() throws IOException {
		...
		final ResourceConfig rc = new PackagesResourceConfig(JerseyRestQueryNetApi.class.getPackage().getName());
