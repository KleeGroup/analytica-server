...
public enum WhatDimension implements Dimension<WhatDimension> {
	 ...
	 */
	WhatDimension(final WhatDimension up, int level) {
		this.up = up;
		this.level = level;
	}
	...
	public List<String> reduce(final List<String> what) {
		Assertion.notNull(what);
		//---------------------------------------------------------------------
		List<String> redux = new ArrayList<String>(getLevel() + 1);
		for (int i = 0; i < getLevel() && what.size() < i; i++) {
			redux.set(i, what.get(i));
		}
		return redux;
	}
