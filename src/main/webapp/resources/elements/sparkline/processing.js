...
(function() {
	...
	var parse = Processing.parse = function parse(aCode, p) {
		...
		aCode = aCode.replace(/(?:static )?(\w+ )(\w+)\s*(\([^\)]*\)\s*{)/g,
				function(all, type, name, args) {
					if (name == "if" || name == "for" || name == "while") {
						return all;
					} else {
						return "Processing." + name + " = function " + name
								+ args;
					}
				});
		...
		aCode = aCode.replace(/([\(,]\s*)(\w+)((?:\[\])+| )\s*(\w+\s*[\),])/g,
				"$1$4");
		aCode = aCode.replace(/([\(,]\s*)(\w+)((?:\[\])+| )\s*(\w+\s*[\),])/g,
				"$1$4");

		// float[] foo = new float[5];
		aCode = aCode.replace(/new (\w+)((?:\[([^\]]*)\])+)/g, function(all,
				name, args) {
		...
		while (intFloat.test(aCode)) {
			aCode = aCode.replace(new RegExp(intFloat), function(all, type,
					name, sep) {
		...
		aCode = aCode.replace(
				/(?:static )?(\w+)((?:\[\])+| ) *(\w+)\[?\]?(\s*[=,;])/g,
				function(all, type, arr, name, sep) {
					if (type == "return")
						return all;
					else
						return "var " + name + sep;
		...
		function ClassReplace(all, name, extend, vars, last) {
			...
			vars = vars.replace(/final\s+var\s+(\w+\s*=\s*.*?;)/g, function(
					all, set) {
				static += " " + name + "." + set;
				return "";
			});
			...
			return "function "
					...
					vars.replace(/,\s?/g, ";\n  this.").replace(
							/\b(var |final |public )+\s*/g, "this.").replace(
							/this.(\w+);/g, "this.$1 = null;")
		...
		while ((m = aCode.match(matchClass))) {
			...
			rest = rest.replace(new RegExp("\\b" + className
					+ "\\(([^\\)]*?)\\)\\s*{", "g"), function(all, args) {
				args = args.split(/,\s*?/);
			...
			rest = rest.replace(
					/(?:public )?Processing.\w+ = function (\w+)\((.*?)\)/g,
					function(all, name, args) {
						return "ADDMETHOD(this, '" + name + "', function("
								+ args + ")";
					});
		...
		aCode = aCode.replace(/Processing.\w+ = function addMethod/g,
				"addMethod");
		...
		// Remove Casting
		aCode = aCode.replace(new RegExp("\\((" + classes.join("|")
				+ ")(\\[\\])?\\)", "g"), "");
	...
	function buildProcessing(curElement) {
		...
		p.color = function color(aValue1, aValue2, aValue3, aValue4) {
			...
			function HSBtoRGB(h, s, b) {
				...
				if (s == 0) {
					...
					var t = Math
							.round((b * (6000 - s * (60 - f))) / 600000 * 255);
		...
		p.AniSprite = function(prefix, frames) {
			...
			for ( var i = 0; i < frames; i++) {
				this.images.push(prefix + p.nf(i, ("" + frames).length)
						+ ".gif");
			}
		...
		function buildImageObject(obj) {
			...
			if (data.__defineGetter__ && data.__lookupGetter__
				...
				data.__defineGetter__("pixels", function() {
					...
					for ( var i = 0; i < pixels.length; i += 4) {
						pixelsDone.push(p.color(pixels[i], pixels[i + 1],
				...
				for ( var i = 0; i < pixels.length; i += 4) {
					data.pixels.push(p.color(pixels[i], pixels[i + 1],
							pixels[i + 2], pixels[i + 3]));
				}
			}
			...
			return {
				...
				width : function(str) {
					if (curContext.mozMeasureText)
						return curContext
								.mozMeasureText(typeof str == "number" ? String
										.fromCharCode(str) : str)
								/ curTextSize;
					else
						return 0;
				}
		...
		p.text = function text(str, x, y) {
			if (str && curContext.mozDrawText) {
				...
				curContext.mozDrawText(typeof str == "number" ? String
						.fromCharCode(str) : str);
				curContext.restore();
			}
		};
		...
		p.curveVertex = function(x, y, x2, y2) {
			if (curvePoints.length < 3) {
				...
				/*
				 * Matrix to convert from Catmull-Rom to cubic Bezier where t =
				 * curTightness |0 1 0 0 | |(t-1)/6 1 (1-t)/6 0 | |0 (1-t)/6 1
				 * (t-1)/6 | |0 0 0 0 |
				 */
		...
		p.float = function float(aNumber) {
			return typeof aNumber == "string" ? p.float(aNumber.charCodeAt(0))
					: parseFloat(aNumber);
		};
		...
		p.random = function random(aMin, aMax) {
			return arguments.length == 2 ? aMin
					+ (Math.random() * (aMax - aMin)) : Math.random() * aMin;
		};

		// From: http://freespace.virgin.net/hugo.elias/models/m_perlin.htm
		p.noise = function(x, y, z) {
			return arguments.length >= 2 ? PerlinNoise_2D(x, y)
					: PerlinNoise_2D(x, x);
		};
		...
		function Noise(x, y) {
			...
			n = (n << 13) ^ n;
			return Math
					.abs(1.0 - (((n * ((n * n * 15731) + 789221) + 1376312589) & 0x7fffffff) / 1073741824.0));
		}
		;

		function SmoothedNoise(x, y) {
			var corners = (Noise(x - 1, y - 1) + Noise(x + 1, y - 1)
					+ Noise(x - 1, y + 1) + Noise(x + 1, y + 1)) / 16;
			var sides = (Noise(x - 1, y) + Noise(x + 1, y) + Noise(x, y - 1) + Noise(
					x, y + 1)) / 8;
		...
		p.get = function get(x, y) {
			...
			if (!getLoaded) {
				getLoaded = buildImageObject(curContext.getImageData(0, 0,
						p.width, p.height));
			}
		...
		p.arc = function arc(x, y, width, height, start, stop) {
			...
			curContext.moveTo(x, y);
			curContext.arc(x, y, curEllipseMode == p.CENTER_RADIUS ? width
					: width / 2, start, stop, false);
		...
		p.rect = function rect(x, y, width, height) {
			...
			curContext.rect(Math.round(x) - offsetStart, Math.round(y)
					- offsetStart, Math.round(width) + offsetEnd, Math
					.round(height)
					+ offsetEnd);
		...
		p.ellipse = function ellipse(x, y, width, height) {
			...
			if (width == height)
				curContext.arc(x - offsetStart, y - offsetStart, width / 2, 0,
						Math.PI * 2, false);
		...
		p.loadPixels = function() {
			p.pixels = buildImageObject(curContext.getImageData(0, 0, p.width,
					p.height)).pixels;
		};
		...
		p.init = function init(code) {
			...
			attach(curElement, "mousemove", function(e) {
				var scrollX = window.scrollX != null ? window.scrollX
						: window.pageXOffset;
			...
			attach(document, "keydown", function(e) {
				...
				if (e.shiftKey) {
					p.key = String.fromCharCode(p.key).toUpperCase()
							.charCodeAt(0);
				}
...
var BaseSparkline = function() {
	...
	this.scale_values = function(values, max) {
		...
		var bottom = (this.scale_from != undefined) ? this.scale_from : this
				.min();
		...
		with (Processing(sl.canvas)) {
			...
			draw = function() {
				...
				if (sl.fill_between_percentage_lines && percentages.length > 1) {
					...
					var height = percentages[percentages.length - 1]
							- percentages[0];
				...
				if (sl.fill_between_value_lines && value_lines.length > 1) {
					...
					var height = value_lines[value_lines.length - 1]
							- value_lines[0];
					var width = scaled[l - 1].x - scaled[0].x;
...
var BarSparkline = function(id, data, mixins) {
	...
	if (!mixins.hasOwnProperty('scale_from'))
		mixins.scale_from = 0;
	this.init(id, data, mixins);
	this.segment_width = function() {
		var l = this.data.length;
	...
	this.scale_width = function() {
		...
		var segment_width = this.segment_width();
		for ( var i = 0; i < l; i++) {
			widths.push((i * segment_width) + (this.padding_between_bars * i)
					+ this.left_padding);
		}
		return widths;
	};
			...
			draw = function() {
				...
				for ( var h = 0; h < value_lines.length; h++) {
					...
					if (sl.extend_markings) {
						...
						line(scaled[0].x, y, scaled[l - 1].x + sw, y);
				}
				var percentages = sl.calc_percentages();
				if (sl.fill_between_percentage_lines && percentages.length > 1) {
					noStroke();
