...
public class ProcessStatsTree implements ProcessStatsCollection<ProcessStatsNode> {
	...
	ProcessStats getProcessStatsByProcesseQueue(final String[] methods) {
		...
		Assertion.checkArgument(methods.length >= 1, "La liste des méthodes ne doit pas étre vide");
		...
		if (methodStatsNode == null) {
			throw new RuntimeException("methodStatsNode ne doit pas étre null");
		}
	...
	public void merge(final ProcessStatsCollection other) {
		...
		Assertion.checkArgument(other instanceof ProcessStatsTree, "On ne peut merger que des ProcessStatsCollection de méme type, impossible de merger {0} avec {1}", this.getClass().getName(), other.getClass().getName());
