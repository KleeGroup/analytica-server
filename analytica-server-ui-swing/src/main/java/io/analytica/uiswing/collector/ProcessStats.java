...
public class ProcessStats implements Serializable {
	...
	public long getDurationsEcartType() {
		// formule ev (non exacte puisque qu'on ne connaét pas toutes les valeurs, mais estimation suffisante)
	...
	public synchronized void merge(final ProcessStats other) {
		...
		Assertion.checkState(processId.equals(other.processId), "On ne peut pas fusionner deux MethodStats qui ne portent pas sur la méme méthode: can''t merge {0} in {1}", other.processId, processId);
		hits += other.hits;
