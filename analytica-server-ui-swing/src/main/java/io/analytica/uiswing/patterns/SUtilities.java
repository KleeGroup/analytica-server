...
public class SUtilities {
	// fichiers é supprimer é la fermeture de l'application (et aussi é la reléve d'opérateur)
	...
	*/
	...
	/**
	 * Trace l'exception et affiche une boéte de dialogue d'erreur.
	...
	public static void handleException(final Throwable throwable) {
		if (throwable instanceof OutOfMemoryError || throwable instanceof RuntimeException || throwable instanceof Error || throwable instanceof UnmarshalException || throwable instanceof ConnectException) {
			// note : SAssertion hérite de Error et est donc considérée aussi comme erreur systéme
			...
			if (isOracleUserException(throwable)) {
				...
				// le message d'erreur é afficher é l'utilisateur est taggé par <text>message</text>
	...
	/**
	 * Trace l'exception et affiche une boéte de dialogue d'erreur systéme.
	 * @param throwable java.lang.Throwable
	 */
	public static void handleError(final Throwable throwable) {
		throwable.printStackTrace();
		// l'affichage des erreurs systémes aux utilisateurs est encapsulée avec le message "Veuillez contacter..."
		if (throwable instanceof RuntimeException && exceptionContains(throwable, "[Erreur sql]")) {
			if (exceptionContains(throwable, "ORA-01089")) {
				...
				handleThrowable(new RuntimeException("Veuillez contacter un administrateur.\n(" + "La base de données SAE est en train de s'arréter.)", throwable.getCause()), true);
			} else {
				// dans kasper.model.ServiceProviderSQL.handleException, kasper masque la sqlexception
				// par une KSystemException et remplace le message oracle par la requéte sql é l'origine de l'erreur
				// (horrible pour un utilisateur qui préférerait le simple message oracle, qui lui peut étre lu é un administrateur,
			...
			handleThrowable(new Exception("L'application ne dispose pas de suffisamment de mémoire pour fonctionner, elle va s'arréter", throwable), true);
			System.exit(2);
		} else {
	 ...
	 * (dans ce cas le message d'erreur é afficher é l'utilisateur est taggé comme suit : <text>message</text>).
	...
	/**
	 * Affiche la boéte de dialogue d'erreur avec MSwing (sauf si il s'agit générateur d'image synoptique)
	 * @param throwable Throwable
	...
	/**
	 * Retourne une imageIcon é partir du cache, en la chargeant auparavant si elle n'y est pas déjé.
	 ...
	 * (sans avoir é instancier une fonte en spécifiant la taille).
	 ...
	 * (donc sans avoir é instancier une fonte avec un simple getDefaultLabelFont().deriveFont(BOLD)).
	 * @return java.awt.Font
	 */
	 ...
	 * <BR>Cette méthode peut-étre trés utile pour récupérer une référence
	 * é un parent éloigné (ancétre), en l'absence de référence directe du type attribut.
	 *
	 ...
	 * la recherche continue é partir du MMasterDetailPanel lié.
	 *
	 ...
	 * <br>Cette méthode peut-étre trés utile pour récupérer une référence
	 * é un parent éloigné (ancétre) de type Dialog ou Frame en l'absence de référence directe du type attribut.
	...
	public static void cancelTimerTaskIfWindowDisposed(final TimerTask task, final Component attachedComponent) {
		...
		if (window == null || !window.isVisible()) {
			...
			// le component et la window (frame ou dialog) seront donc éventuellement garbage collectés é ce moment lé
			task.cancel();
		}
	}
